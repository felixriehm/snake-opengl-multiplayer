    ...
    public static class PointRegionQuadTree<P extends QuadTree.XYPoint> extends QuadTree<P> {
        ...
        protected static class PointRegionQuadNode<XY extends QuadTree.XYPoint> extends QuadNode<XY> {
            ...
            protected Pair<XY, Boolean> insertGameData(XY p) {
                ...
                if(isLeaf() && points.contains(p)) {
                    return new Pair<>(points.get(points.indexOf(p)), false); // object cannot be added
                }
            ...
            public boolean insertIntoChildren(XY p) {
            ...
            private Pair<XY,Boolean> insertGameDataIntoChildren(XY p) {
                ...
                if(result == null) {
                    // point cant be added because the point is outside
                    // of the region. Return point is null.
                    return new Pair<>(null, false);
                } else {
                    // point cant be added because a point with the same
                    // coordinate is already there. Return is the point which
                    // already exists. Second tupel value is false.
                    return result;
                }
            }
